...
void access_data(mem_addr_t addr) {                      
    ...
    cache_line_t* curr = cache[set];
    cache_line_t* prev = curr;;
    // checking for cache hit 

    while (curr != NULL){
        if (curr->valid != '0' && curr->tag == tag){
            hit_cnt++;
            // change position to front of the list
            if (curr != cache[set]){
                prev->next = curr->next;
                curr->next = cache[set];
                cache[set] = curr; 
    ...
    // miss - will be reached only if no hit
    // find an empty space in the cache set
    ...
    while (curr != NULL){
        if (curr->valid == '0'){
            curr->tag = tag;
            curr->valid = '1';
            // bringing the line to the front of the linked list
    ...
    cache_line_t* new = malloc(sizeof(cache_line_t));
    if (new == NULL){
        ...
        if (buf[1] == 'S' || buf[1] == 'L' || buf[1] == 'M') {
            ...
            switch (buf[1]){
                case 'S': access_data(addr);
                          break;
                case 'L': access_data(addr);
                          break;
                case 'M': access_data(addr);
                          access_data(addr);
                          break;
                }
            if (verbosity)
